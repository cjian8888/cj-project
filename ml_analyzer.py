...
EXCLUDE_NODES_EXTENDED = {
    # 支付平台
    '支付宝', '微信', '财付通', '网银在线', '京东支付', '银联', '翼支付', '云闪付',
    '抖音支付', '快捷支付', '非银行支付机构', '数字货币', '网联',
    
    # 电商/外卖/出行
    '美团', '饿了么', '滴滴出行', '携程', '去哪儿', '飞猪', '高德', '百度地图',
    '京东', '淘宝', '天猫', '拼多多', '唯品会', '苏宁', '国美',
    
    # 餐饮连锁
    '金拱门', '麦当劳', '肯德基', '必胜客', '星巴克', '瑞幸', '奶茶', '咖啡',
    
    # 交通运输
    '铁路', '航空', '航旅', '12306', '地铁', '公交', '出租', '加油站',
    '中国石油', '中国石化', '壳牌', '充电', '停车',
    
    # 医疗
    '医院', '诊所', '药房', '药店', '卫生', '妇幼', '保健',
    
    # 政府/公共服务
    '税务', '社保', '公积金', '水电', '燃气', '物业', '电信', '移动', '联通',
    
    # 金融机构（非涉案）
    '银行', '信托', '基金', '保险', '证券', '理财', '贷款',
    
    # 通用排除词
    '手续费', '利息', '工资', '社保', '公积金', '消费', '提现', '充值', 
    '退款', '红包', '转账', '零钱', '余额宝', '零钱通',
    
    # 物流快递
    '顺丰', '圆通', '中通', '韵达', '申通', '邮政', '快递', '丰巢',
...
class LightweightAnomalyDetector:
    ...
    def fit_predict(self, df: pd.DataFrame, features: List[str], 
                    raw_mapping: Dict = None) -> pd.Series:
        """
        训练并预测异常分数
...
def _feature_engineering(
    ...
    for person in core_persons:
        ...
        for _, row in df.iterrows():
            try:
                # 获取金额 (支持 amount 或 income+expense)
                amount = 0.0
                val_amt = get_val(row, 'amount')
                if val_amt is not None:
                    amount = float(val_amt)
                else:
                    inc = float(get_val(row, 'income') or 0)
                    exp = float(get_val(row, 'expense') or 0)
                    amount = inc + exp
                ...
                cp = str(get_val(row, 'counterparty') or '')
                desc = str(get_val(row, 'description') or '')
                
                # 特征 1: 金额对数 (解决金额长尾分布问题)
                amount_log = np.log1p(amount)
                
                # 特征 2 & 3: 时间周期编码 (保留时序距离)
                hour = date.hour
                hour_sin = np.sin(2 * np.pi * hour / 24)
                hour_cos = np.cos(2 * np.pi * hour / 24)
                
                # 特征 4: 是否为整数 (异常交易往往是整数)
                is_round = 1.0 if amount % 100 == 0 else 0.0
                
                # 特征 5: 对手方频率编码 (罕见对手方更可疑)
                # 取倒数，频率越低值越大
                freq = cp_counts.get(cp, 1)
                freq_enc = 1.0 / np.log1p(freq)
                
                # 特征 6: 摘要长度 (过短的摘要可能掩盖意图)
                desc_len = len(desc)
...
def run_ml_analysis(
    ...
    logger.info(f'核心实体: {len(core_entities)} 个 ({len(core_persons)}人 + {len(involved_companies)}公司)')
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        if communities:
            ...
            for i, comm in enumerate(communities, 1):
                ...
                f.write(f"  核心成员: {', '.join(comm.get('core_members', comm['members']))}\n")
                f.write(f"  成员数量: {comm['count']} 人 (其中核心 {comm.get('core_count', '?')} 人)\n")
                f.write(f"  闭环交易总额: {utils.format_currency(comm['total_amount'])}\n")
                ...
                if edge_details:
                    ...
                    for edge in edge_details[:5]:
                        f.write(f"    {edge['from']} -> {edge['to']}: {utils.format_currency(edge['amount'])}\n")
                f.write("\n")
        else:
            f.write('二、可疑资金团伙分析\n')
            f.write('-'*40 + '\n')
        ...
        for i, item in enumerate(anomalies[:50], 1):  # 只显示前50个
            ...
            f.write(f"   摘要: {item['description'][:50]}{'...' if len(item['description']) > 50 else ''}\n")
