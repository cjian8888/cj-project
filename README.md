...
- **HTML 综合报告** - 详细的文字分析报告
- **电子钱包补充分析报告** (`电子钱包补充分析报告.txt`) - 微信 / 支付宝 / 财付通补充样本的独立文字分析
- **电子钱包补充清洗表** (`电子钱包补充清洗表.xlsx`) - 电子钱包补充样本的规范化清洗明细与预警工作簿
- **电子钱包重点核查清单** (`电子钱包重点核查清单.txt`) - 仅保留高风险预警与未归并微信账号的轻量清单
...
系统已将微信 / 支付宝 / 财付通协查数据定义为“后补补充数据源”，而不是银行流水主清洗链的一部分。

- 输入根目录可以是前端任意选择的 `data` 目录；程序会从该根目录向下递归扫描全部子目录
- 推荐目录：`<inputDirectory>/补充数据/电子钱包/批次_YYYYMMDD/`
- 目录识别规则：电子钱包补充目录建议命名包含 `电子钱包`、`wallet`、`ewallet`、`zfb+wx`、`wx+zfb` 之一，银行主链扫描会跳过这类目录，电子钱包提取器仍会递归识别其中样本
- 接入方式：专用提取器解析后写入 `output/analysis_cache/walletData.json`
- 专项产物：额外输出 `output/analysis_results/电子钱包补充分析报告.txt`、`output/analysis_results/电子钱包补充清洗表.xlsx`、`output/analysis_results/电子钱包重点核查清单.txt`
- 前端辅助：补充数据页支持手工导出“当前重点核查清单”精简版，用于临时汇报或人工跟进
- 设计原则：无电子钱包目录时主程序行为不变；有电子钱包目录时仅增强补充分析，不改写银行主链结论
- 多目录策略：同一轮分析下若放入多个电子钱包子目录，程序会按预期自动合并扫描，把它们视为同一轮补充证据样本
- 当前落地：前端“补充数据”页、风险情报 `walletAlerts`、图谱页电子钱包关系边、聚合评分证据包、正式报告“电子钱包补充核查”章节

专项说明见 [docs/电子钱包补充数据接入方案.md](docs/电子钱包补充数据接入方案.md)
...
- 微信 / 支付宝 / 财付通协查数据不写入 `output/cleaned_data/`
- 它们作为“后补补充数据”进入 `output/analysis_cache/`
- 输入根目录可自定义，推荐放入 `inputDirectory/补充数据/电子钱包/批次_YYYYMMDD/`，并由专用提取器处理
- 多个电子钱包子目录会自动合并扫描
- 同时会在 `output/analysis_results/` 目录生成电子钱包专项 TXT、轻量核查清单与 Excel 产物，便于单独审阅
...
> 📖 **技术参考**：详细的算法设计和检测逻辑请参阅 [docs/TECHNICAL_REFERENCE.md](docs/TECHNICAL_REFERENCE.md)

### 版本历程

```
v4.5.x (2026-01-19) ─── 当前版本
  ├─ UI/UX 优化：快捷导航折叠、黑屏修复
  ├─ 数据溯源：精确到文件名和行号
  └─ 构建优化：简化构建流程

v4.4.0 (2026-01-18) ─── 代码质量审计
  ├─ P0-P5 六阶段系统性改进
  ├─ 新增：操作审计日志、节假日服务、Neo4j 适配器
  └─ 报告追加账户/银行/文件溯源信息

v4.3.0 (2026-01-17) ─── 数据铁律重构
  ├─ /api/results 改为从缓存读取
  └─ 新增分析缓存机制

v4.2.0 (2026-01-16) ─── 离线环境适配
  ├─ 移除外部 CDN 依赖
  └─ vis-network 本地内联

v4.1.0 (2026-01-11) ─── 线索聚合引擎
  ├─ 新增 clue_aggregator.py
  ├─ 以实体为中心的证据包视图
  └─ 风险分归因解释
...
v3.2.0 (2026-01-10) ─── 功能完善
  ├─ 修复理财产品误判
  └─ 增加政府机关白名单

v3.0.0 (2026-01-07) ─── 增强版 📌
  ├─ 借贷行为分析（配对、无还款、异常利息）
  ├─ 异常收入来源检测
  ├─ 关联方资金穿透
  └─ 多源数据交叉碰撞

v2.0.0 (2026-01-06) ─── 多银行支持
  ├─ 多银行格式自动识别
  └─ 资产全链条追踪
