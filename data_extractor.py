...
def extract_clues_from_pdf(pdf_path: str) -> Tuple[List[str], List[str]]:
    ...
    if file_size > 50 * 1024 * 1024:  # 50MB
        logger.warning(f'PDF文件过大({file_size/1024/1024:.1f}MB)，跳过处理: {pdf_path}')
        return [], []
...
def find_column_by_keywords(df: pd.DataFrame, keywords: List[str],
                            min_score: int = 60, verbose: bool = True) -> str:
    """
    ...
    for keyword in keywords:
        ...
        for col in columns:
            ...
            score, match_type = _calculate_match_score(normalized_col, normalized_keyword)
    ...
    if verbose and all_candidates:
        ...
        for i, candidate in enumerate(all_candidates[:5]):  # 只显示前5个
            logger.debug(f"  {i+1}. 列名='{candidate['column']}', "
    ...
    if best_score >= min_score:
        if verbose:
            logger.info(f"列名匹配成功: '{best_match}' (分数={best_score}, "
                       f"类型={best_match_type}, 关键词='{keywords}')")
        return best_match
    else:
        if verbose:
            if best_match:
                logger.warning(f"列名匹配分数过低: '{best_match}' (分数={best_score} < 阈值={min_score}), "
                             f"关键词='{keywords}', 可用列: {columns}")
            else:
                logger.warning(f"未找到匹配的列名，关键词='{keywords}', 可用列: {columns}")
        return None
...
def read_excel_transactions(excel_path: str) -> pd.DataFrame:
    ...
    if file_size > 100 * 1024 * 1024:  # 100MB
        logger.warning(f'Excel文件过大({file_size/1024/1024:.1f}MB)，跳过处理: {excel_path}')
        return pd.DataFrame()
    ...
    try:
        ...
        if header_row > 0:
            try:
                ...
                logger.warning(f'使用表头行{header_row}读取失败: {str(e)}，使用默认表头')
    ...
    try:
        for root, dirs, files in os.walk(directory):
            for filename in files:
                ...
                if file_size > 100 * 1024 * 1024:  # 100MB
                    logger.warning(f'文件过大({file_size/1024/1024:.1f}MB)，跳过: {filename}')
                    continue
        ...
        logger.info(f'共加载 {len(all_transactions)} 个流水文件 (成功: {processed_files}, 失败: {failed_files})')
    ...
    try:
        for root, dirs, files in os.walk(directory):
            for filename in files:
                ...
                file_size = os.path.getsize(file_path)
                if file_size > 50 * 1024 * 1024:  # 50MB
                    logger.warning(f'PDF文件过大({file_size/1024/1024:.1f}MB)，跳过: {filename}')
                    continue
        ...
        logger.info(f'共找到 {len(clue_files)} 个线索文件 (成功: {processed_files}, 失败: {failed_files})')
...
def extract_all_clues(directory: str) -> Tuple[List[str], List[str]]:
    ...
    logger.info(f'汇总提取: {len(all_persons)} 个人名, {len(all_companies)} 个公司名 (成功: {processed_files}, 失败: {failed_files})')
