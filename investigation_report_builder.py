...
from report_schema import (
    ...
    # v3.1 新增：征信预警与行为风险
    CreditAlertItem, BehavioralRiskItem, BehavioralRiskSummary, SuspiciousFlags,
    # v3.1 新增：家庭分析层级结构
    RelationEvidence, FamilyRosterMember, FamilyRoster,
    IncomeAssetMismatch, JointRisks, FamilyLevelAnalysis,
    FamilyUnit, HouseholdAnalysisSection,
    # v4.0 新增：报告骨架结构
    FamilyAssetsOverview, FamilyOverviewSection, FamilyYearlySalary,
    PersonalFinancialProfile, DimensionAnalysis
)
...
class InvestigationReportBuilder:
    ...
    def __init__(self, analysis_cache: Dict, output_dir: str = './output'):
        ...
        logger.info(f"[初查报告构建器] 外部数据缓存: 房产{len(self._external_property_cache)}, 车辆{len(self._external_vehicle_cache)}, 理财{len(self._external_wealth_cache)}, 证券{len(self._external_securities_cache)}")
    ...
    def _build_id_name_mapping(self, analysis_cache: Dict):
        ...
        for person_id, vehicles in vehicle_data.items():
            ...
            if isinstance(vehicles, list) and vehicles:
                ...
                owner_name = first_vehicle.get('owner_name', '') or first_vehicle.get('name', '')
    ...
    def _build_household_analysis_section(self, config: PrimaryTargetsConfig) -> HouseholdAnalysisSection:
        """
    ...
    def _build_family_roster(self, anchor: str, members: List[str]) -> FamilyRoster:
        ...
        householder = matched_unit.get('householder', anchor) if matched_unit else anchor
        address = matched_unit.get('address', '') if matched_unit else ''
        member_details_raw = matched_unit.get('member_details', []) if matched_unit else []
        extended_relatives = matched_unit.get('extended_relatives', []) if matched_unit else []
        ...
        for name in members:
            ...
            roster_members.append(FamilyRosterMember(
                name=name,
                relation=relation,
                has_data=name in self.profiles,
                ...
                relation_evidence=RelationEvidence(
                    ...
                    source="户籍登记信息" if evidence_type == "户籍确认" else "数据推断",
    ...
    def _build_family_level_analysis(self, members: List[str]) -> FamilyLevelAnalysis:
        ...
        # 1. 收支资产匹配分析
        income_asset_mismatch = self._build_income_asset_mismatch(members, family_summary)
        
        # 2. 内部互转风险分析
        joint_risks = self._build_joint_risks(members, family_summary)
        
        return FamilyLevelAnalysis(
            income_asset_mismatch=income_asset_mismatch,
            joint_risks=joint_risks
        )
    
    def _build_income_asset_mismatch(self, members: List[str], family_summary: Dict) -> IncomeAssetMismatch:
        """构建收支资产匹配分析"""
        total_income = family_summary.get('total_income_expense', {}).get('total_income', 0) or 0
        total_expense = family_summary.get('total_income_expense', {}).get('total_expense', 0) or 0
        ...
        if income_covers_expense and income_covers_assets:
            analysis_text = "家庭总收入能够覆盖总支出及资产购置，未发现明显收支不抵情况。"
            risk_level = "low"
    ...
    def _build_joint_risks(self, members: List[str], family_summary: Dict) -> JointRisks:
        """构建家庭成员间联合风险"""
        ...
        # 判断是否异常（内部互转超过总收入的50%）
        total_income = family_summary.get('total_income_expense', {}).get('total_income', 1)
        has_anomaly = internal_transfer_total > total_income * 0.5 if total_income else False
    ...
    def _build_family_overview_section(self, members: List[str], anchor: str) -> FamilyOverviewSection:
        """
        【v4.0】构建家庭整体分析章节
        ...
        narrative = self._generate_family_overview_narrative(
            family_name, len(members), total_income, total_expense, 
            total_salary, assets_overview
        )
    ...
    def _calculate_family_assets_total(self, members: List[str]) -> FamilyAssetsOverview:
        """
        【v4.0】计算家庭资产总和
        ...
        for name in members:
            ...
            for acc in bank_accounts:
                balance = acc.get('余额', 0) or acc.get('balance', 0) or acc.get('账户余额', 0) or 0
                if isinstance(balance, (int, float)) and balance > 0:
                    bank_balance += balance
            
            # 理财持仓
            wealth_total = profile.get('wealthTotal', 0) or profile.get('wealth_total', 0) or 0
        ...
        narrative = self._generate_family_assets_narrative_text(
            property_count, property_value, bank_balance, wealth_holding, vehicle_count, total_assets
        )
    ...
    def _aggregate_family_yearly_salary(self, members: List[str]) -> List[FamilyYearlySalary]:
        """
        【v4.0】汇总家庭年度工资
        ...
        for name in members:
            ...
            # 尝试从 salaryByYear 获取年度工资数据
            salary_by_year = profile.get('salaryByYear', {}) or profile.get('salary_by_year', {}) or {}
            
            for year, amount in salary_by_year.items():
                if year not in yearly_data:
                    yearly_data[year] = {}
                yearly_data[year][name] = amount or 0
        ...
        for year in sorted(yearly_data.keys()):
            ...
            result.append(FamilyYearlySalary(
                year=str(year),
                total=total,
    ...
    def _build_personal_financial_profile(self, name: str, profile: Dict) -> PersonalFinancialProfile:
        """
        【v4.0】构建个人资金特征画像
        ...
        professional_narrative = self._generate_professional_narrative(
            name, profile, total_score, risk_level,
            income_expense_match, lending_behavior, consumption_pattern,
            fund_flow, cash_operation
        )
        
        conclusion_narrative = self._generate_conclusion_narrative(
            name, risk_level, total_score,
            [income_expense_match, lending_behavior, consumption_pattern, 
             fund_flow, cash_operation]
        )
        ...
        # 收集高风险预警
        high_risk_alerts = []
        for dim in [income_expense_match, lending_behavior, consumption_pattern, 
                    fund_flow, cash_operation]:
    ...
    def _analyze_income_expense_match(self, name: str, profile: Dict) -> DimensionAnalysis:
        """分析收支匹配度"""
        total_income = profile.get('totalIncome', 0) or 0
        total_expense = profile.get('totalExpense', 0) or 0
        salary_total = profile.get('salaryTotal', 0) or 0
        ...
        if gap > 0:
            ...
            if gap_ratio > 50:
                score = 25
                risk_level = "high"
                findings.append(f"支出超过收入{gap/10000:.1f}万元（超支{gap_ratio:.0f}%），存在重大收支不抵")
            elif gap_ratio > 20:
                score = 15
                risk_level = "medium"
                findings.append(f"支出超过收入{gap/10000:.1f}万元（超支{gap_ratio:.0f}%），需核实资金来源")
            else:
                score = 8
        ...
        if salary_ratio < 30 and total_income > 500000:
            ...
            findings.append(f"工资收入仅占总收入{salary_ratio:.0f}%，非工资收入占比较大")
    ...
    def _analyze_consumption_pattern(self, name: str, profile: Dict) -> DimensionAnalysis:
        """分析消费特征"""
        # 获取大额支出数据
        ...
        if score == 0:
            ...
            narrative = f"{name}存在{luxury_count}笔疑似高档消费，建议结合其收入水平综合判断。"
    ...
    def _analyze_cash_operation(self, name: str, profile: Dict) -> DimensionAnalysis:
        ...
        if large_cash_count >= 5 or large_cash_total > 500000:
            score = 15
            risk_level = "high"
            findings.append(f"存在大额现金操作{large_cash_count}笔，合计{large_cash_total/10000:.1f}万元")
    ...
    def _generate_professional_narrative(self, name: str, profile: Dict, 
                                         total_score: int, risk_level: str,
                                         *dimensions) -> str:
        """
        【v4.0】生成纪检视角专业审计话术
        ...
        if risk_level == "low":
            ...
            concern_text = "；".join(concerns[:3]) if concerns else "部分资金流向需进一步核实"
            return (
            ...
            risk_text = "；".join(major_risks[:3]) if major_risks else "存在重大资金异常"
            return (
                f"经核查，{name}存在以下重大疑点：{risk_text}。"
                f"上述异常情况与廉洁规定存在潜在冲突，建议立即采取措施进一步核查。"
            )
    
    def _generate_conclusion_narrative(self, name: str, risk_level: str, 
                                       total_score: int, dimensions: List) -> str:
        """生成综合结论话术"""
        ...
        dim_summaries = []
        for dim in dimensions:
            if hasattr(dim, 'dimension_name') and hasattr(dim, 'score') and hasattr(dim, 'max_score'):
                dim_summaries.append(f"{dim.dimension_name}{dim.score}/{dim.max_score}分")
    ...
    def _generate_family_overview_narrative(self, family_name: str, member_count: int,
                                            total_income: float, total_expense: float,
                                            total_salary: float, assets_overview: FamilyAssetsOverview) -> str:
        """生成家庭整体分析综述话术"""
        return (
    ...
    def _generate_family_assets_narrative(self, assets_overview: FamilyAssetsOverview) -> str:
        """生成家庭资产话术"""
        parts = []
        if assets_overview.property_count > 0:
            parts.append(f"房产{assets_overview.property_count}套（估值{assets_overview.property_value_wan:.1f}万元）")
        else:
    ...
    def _generate_family_salary_narrative(self, members: List[str], total_salary: float,
                                          avg_yearly_salary: float, 
                                          yearly_salaries: List[FamilyYearlySalary]) -> str:
        """生成家庭工资收入话术"""
    ...
    def _generate_family_assets_narrative_text(self, property_count: int, property_value: float,
                                               bank_balance: float, wealth_holding: float,
                                               vehicle_count: int, total_assets: float) -> str:
        """生成家庭资产描述文本"""
        return (
    ...
    def _build_suspicious_flags(self, name: str, profile: Dict, large_cash: LargeCashAnalysis) -> SuspiciousFlags:
        """
        构建可疑标记汇总（v3.1新增）
        ...
        # 4. 可疑对手方转账（借贷异常等）
        suspicious_counterparty_transfers = self._get_suspicious_counterparty_transfers(name)
    ...
    def _get_credit_alerts_for_person(self, name: str) -> List[CreditAlertItem]:
        ...
        for alert in credit_alerts_raw:
            if alert.get('name') == name:
                person_alerts.append(CreditAlertItem(
                    name=alert.get('name', ''),
    ...
    def _get_behavioral_risk_for_person(self, name: str) -> BehavioralRiskSummary:
        ...
        high_risk_items = [
            BehavioralRiskItem(
                ...
                counterparty=item.get('large_counterparty', '') or item.get('counterparty', ''),
        ...
        return BehavioralRiskSummary(
            ...
            total_patterns=len(person_fast_in_out) + len(person_structuring) + len(person_dormant),
    ...
    def build_report_v4(self,
        ...
        if config:
            ...
            try:
                ...
                loaded_config, load_msg = service.load_config()  # 返回(config, message)元组
                if loaded_config and hasattr(loaded_config, 'analysis_units') and loaded_config.analysis_units:
                    logger.info(f"[初查报告v4] 自动加载配置文件，包含 {len(loaded_config.analysis_units)} 个分析单元")
                    all_persons = []
                    families = {}
                    for unit in loaded_config.analysis_units:
                        ...
                        if members_in_unit:
                            families[anchor] = {
                                ...
                                'members': members_in_unit,
                                'unit_type': unit.unit_type if hasattr(unit, 'unit_type') else 'family',
                                'member_details': unit.member_details if hasattr(unit, 'member_details') else [],
                            }
                    include_companies = loaded_config.include_companies or self._companies.copy()
                else:
    ...
    def _build_family_summary_v4(self, anchor: str, members: List[str]) -> Dict:
        ...
        if use_cache:
            ...
            logger.debug(f"[家庭汇总] 成员不匹配，从 profiles 计算: cached={cached_members}, current={current_members}")
            total_income = 0.0
            total_expense = 0.0
            ...
            for member in members:
                ...
                # 银行账户余额
                accounts = profile.get('bankAccounts', []) or profile.get('bank_accounts', [])
                if isinstance(accounts, list):
                    for acc in accounts:
                        total_bank_balance += acc.get('last_balance', 0) or acc.get('balance', 0) or 0
    ...
    def _build_v4_preface(self, persons: List[str], companies: List[str], case_background: str = None) -> Dict:
        ...
        date_range = self.metadata.get('date_range', {})
        if date_range:
            start_date = date_range.get('start', '')[:10] if date_range.get('start') else ''
        ...
        if not start_date or not end_date:
            family_summary = self.derived_data.get('familySummary', {}) or self.derived_data.get('family_summary', {})
            if family_summary:
                meta = family_summary.get('_meta', {})
                if meta:
                    start_date = start_date or meta.get('start_date', '')[:10] if meta.get('start_date') else ''
                    end_date = end_date or meta.get('end_date', '')[:10] if meta.get('end_date') else ''
    ...
    def _generate_enhanced_summary_text_v4(self, issues: List[Dict], risk_levels: Dict) -> str:
        ...
        for i, issue in enumerate(issues, 1):
            risk_label = {'high': '【高风险】', 'medium': '【中风险】', 'low': '【低风险】'}.get(issue['severity'], '')
...
"""
    ...
    def _build_v4_next_steps(self, person_sections: List[Dict], company_sections: List[Dict]) -> List[Dict]:
        ...
        for section in person_sections:
            ...
            if income_match.get('need_further_verification'):
                salary_ratio = income_match.get('salary_ratio', 0)  # 这里已经是百分比形式如24.0
    ...
    def _build_property_info_v4(self, name: str, profile: Dict) -> Dict:
        ...
        if not properties:
            ...
            if person_id:
                ...
                if properties:
                    logger.info(f"[房产数据] {name} 从外部缓存获取 {len(properties)} 条房产记录")
    ...
    def _get_external_vehicle_data(self, name: str) -> List[Dict]:
        ...
        person_id = self._name_to_id_map.get(name) or self._external_name_to_id_map.get(name)
        if person_id:
    ...
    def _get_external_property_data(self, name: str) -> List[Dict]:
        ...
        # 优先使用__init__建立的映射
        person_id = self._name_to_id_map.get(name) or getattr(self, '_external_name_to_id_map', {}).get(name)
        if person_id:
    ...
    def _build_large_cash_analysis_v4(self, name: str, profile: Dict) -> Dict:
        ...
        return {
            ...
            "transactions": [self._clean_transaction(t) for t in cash_transactions[:20]],  # 【审计修复】清理nan
...
def load_investigation_report_builder(output_dir: str = './output') -> Optional[InvestigationReportBuilder]:
    ...
    if os.path.exists(full_cache_path):
        try:
            ...
            logger.info(f"[初查报告] 缓存包含: {', '.join(list(analysis_cache.keys())[:10])}")
        ...
        if key == 'profiles':
            ...
            if os.path.exists(full_filepath):
                try:
                    ...
                    logger.warning(f"[初查报告] 加载 profiles_full.json 失败: {e}，回退到简化版")
