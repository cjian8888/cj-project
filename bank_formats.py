...
def detect_bank_format(df: pd.DataFrame, verbose: bool = True) -> str:
    ...
    if best_score < dynamic_threshold:
        if verbose:
            logger.warning(f'银行格式检测分数过低: {best_score:.2f} < 阈值={dynamic_threshold:.2f}, '
                         f'使用通用格式')
        best_match = 'GENERIC'
    
    # 输出详细日志
    if verbose:
        logger.info(f'银行格式检测结果: {best_match} ({BANK_FORMATS[best_match]["name"]})')
        ...
        for i, score_info in enumerate(all_scores[:5]):  # 只显示前5个
            ...
            logger.debug(f"  {i+1}. {score_info['bank_code']} ({score_info['bank_name']}): "
        ...
        if best_details and best_match != 'GENERIC':
            ...
            for expected_col, match_info in best_details['matched_columns'].items():
                logger.debug(f"  {expected_col} -> {match_info['matched_to']} "
...
def normalize_dataframe(df: pd.DataFrame, bank_format: str = None) -> pd.DataFrame:
    ...
    # 【修复】添加列名存在性检查
    if not rename_map:
        logger.warning(f'银行格式 {bank_format} 的列映射为空，使用原始列名')
        return df
    ...
    if 'date' in result_df.columns and config['date_format']:
        try:
            ...
            if date_parse_failures > 0:
                logger.warning(f'日期解析失败 {date_parse_failures} 条，已转为NaT')
        ...
        if col in result_df.columns:
            try:
                ...
                if conversion_failures > 0:
                    logger.warning(f'{col}列转换失败 {conversion_failures} 条，已填充为0')
            except Exception as e:
