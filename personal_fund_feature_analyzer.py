...
class PersonalFundFeatureAnalyzer:
    ...
    def to_wan_yuan(amount: float, unit: str = 'fen') -> float:
        ...
        """
        if unit == 'fen':
            return amount / 1000000  # 分 → 万元 (1000000分 = 10000元 = 1万元)
        elif unit == 'yuan':
    ...
    def _analyze_income_expense_match(self,
        ...
        # 计算消费支出（包含取现）
        expense_mask = transactions["transaction_type"].isin(["消费", "转账支出", "取现", "提现"])
        # 交易数据是分，需要转换为万元
    ...
    def _analyze_consumption_pattern(self,
        ...
        consume_mask = transactions["transaction_type"].isin([
            "消费", "转账支出", "取现", "提现"
        ])
        # 排除投资、理财、还款等非消费性支出
