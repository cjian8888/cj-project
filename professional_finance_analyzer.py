...
class FinancialProductAnalyzer:
    ...
    def analyze(
        ...
        # 3. 分析理财规模和收益
        finance_scale = self._analyze_finance_scale_and_income(
            finance_txns, income_yearly, person_profile
        )
    ...
    def _identify_financial_transactions(
        ...
        # 识别理财交易
        finance_mask = transactions["description"].str.contains(
            finance_pattern, na=False, case=False
        )
    ...
    def _analyze_product_types(self, finance_txns: pd.DataFrame) -> Dict[str, Any]:
        ...
        # 统计各类理财产品的交易笔数和金额
        for category, keywords in FINANCIAL_PRODUCT_KEYWORDS.items():
            category_mask = finance_txns["description"].str.contains(
            ...
            product_types[category] = {
                ...
                "avg_amount": category_amount / category_count
                if category_count > 0
                else 0,
            }
        ...
        # 生成描述
        main_types = sorted(
            product_types.items(), key=lambda x: x[1]["amount"], reverse=True
        )[:3]
        main_types_str = "、".join(
            [f"{k}({v['count']}笔)" for k, v in main_types if v["count"] > 0]
        )
    ...
    def _analyze_finance_scale_and_income(
        self,
        finance_txns: pd.DataFrame,
        income_yearly: Dict[int, float],
        person_profile: Dict[str, Any],
    ) -> Dict[str, Any]:
        """分析理财规模和收入对比"""
        ...
        # 计算理财收入（理财收益）
        # 假设理财赎回金额 - 申购金额 = 理财收益
        redemption_mask = finance_txns["description"].str.contains(
            "赎回", na=False, case=False
        )
        purchase_mask = finance_txns["description"].str.contains(
            "申购|购入", na=False, case=False
        )

        redemption_amount = (
            finance_txns[redemption_mask]["amount"].sum()
            if not finance_txns[redemption_mask].empty
            else 0
        )
        purchase_amount = (
            finance_txns[purchase_mask]["amount"].sum()
            if not finance_txns[purchase_mask].empty
            else 0
        )

        finance_income = (
            max(0, (redemption_amount - purchase_amount)) / 10000
        )  # 转为万元
        ...
        if finance_ratio > 10:
            ...
            description = (
                f"该人员理财规模{total_finance:.2f}万元，理财资产与其收入水平基本匹配。"
            )
    ...
    def _analyze_holding_period(self, finance_txns: pd.DataFrame) -> Dict[str, Any]:
        """分析理财持有期（短期 vs 长期）"""
        # 识别短期理财产品（持有期 < 1年）
        short_term_mask = finance_txns["description"].str.contains(
            "短期|T+|T+[0-9]|活期|七日|一月|三月|六月", na=False, case=False
        )
        ...
        # 识别长期理财产品（持有期 > 3年）
        long_term_mask = finance_txns["description"].str.contains(
            "三年|五年|十年|长期|定期|封闭式", na=False, case=False
        )
        ...
        for account in finance_txns["account_number"].unique():
            ...
            for i in range(len(account_txns) - 1):
                ...
                if time_diff <= 7:
                    ...
                    if (
                        curr_txn["direction"] == "income"
                        and next_txn["direction"] == "expense"
                    ) or (
                        curr_txn["direction"] == "expense"
                        and next_txn["direction"] == "income"
                    ):
                        # 检查金额是否基本相等（允许10%的偏差）
                        amount_ratio = min(
                            curr_txn["amount"], next_txn["amount"]
                        ) / max(curr_txn["amount"], next_txn["amount"], 1)
    ...
    def _analyze_asset_reasonability(
        self,
        finance_scale: Dict,
        product_types: Dict,
        ...
        # 资产与工资对比
        assets_to_salary_ratio = (
            total_assets / (annual_income + 1) if annual_income > 0 else 0
        )
    ...
    def _generate_audit_descriptions(self, dimensions: Dict) -> List[str]:
        ...
        if finance_scale["score"] >= 20:
            descriptions.append(
                f"该人员理财规模{finance_scale['total_finance_amount']:.2f}万元，累计理财收益{finance_scale['total_finance_income']:.2f}万元，理财资产是其年度工资收入的{finance_scale['finance_ratio']:.1f}倍，远超正常水平，存在资产来源不明风险，需重点核查其理财资金来源。"
            )
        elif finance_scale["score"] >= 10:
            descriptions.append(
                f"该人员理财规模{finance_scale['total_finance_amount']:.2f}万元，理财资产是其年度工资收入的{finance_scale['finance_ratio']:.1f}倍，理财积累明显超过收入水平，需关注其资金来源。"
            )
        ...
        if product_types["score"] >= 20:
            main_types = ", ".join(
                [
                    f"{k}({v['count']}笔)"
                    for k, v in product_types["main_types"]
                    if v["count"] > 0
                ]
            )
            descriptions.append(
                f"该人员持有{product_types['types_count']}种不同类型理财产品，包括{main_types}，投资结构较为复杂，需核查其投资风险承受能力及资产配置合理性。"
            )
            ...
            descriptions.append(
                f"该人员持有{product_types['types_count']}种不同类型理财产品，包括{main_types}，投资结构较为多样，需关注其投资配置合理性。"
            )
        ...
        if holding_period["score"] >= 20:
            descriptions.append(
                f"该人员理财产品持有期普遍较短，短期产品占比{holding_period['short_term_count'] / holding_period['total_finance_count']:.1%}，存在资金快进快出特征，需关注其资金用途和背景。"
            )
        elif holding_period["score"] >= 10:
            descriptions.append(
                f"该人员短期理财产品占比较高，占比{holding_period['short_term_count'] / holding_period['total_finance_count']:.1%}，存在一定的资金周转需求。"
            )
        ...
        if cycling["score"] >= 20:
            descriptions.append(
                f"该人员资金在理财产品间频繁流转，累计{cycling['cycle_count']}次资金循环，涉及{cycling['cycle_amount']:.2f}万元，存在明显的资金空转行为，需重点关注。"
            )
        elif cycling["score"] >= 10:
            descriptions.append(
                f"该人员资金在理财产品间存在一定程度的流转，累计{cycling['cycle_count']}次资金循环，涉及{cycling['cycle_amount']:.2f}万元，需关注其资金用途。"
            )
        ...
        if asset_reasonability["score"] >= 30:
            descriptions.append(
                f"该人员资产总额{asset_reasonability['total_assets'] / 10000:.2f}万元，是其年度工资收入的{asset_reasonability['assets_to_salary_ratio']:.1f}倍，资产积累明显超过收入水平，可能存在隐匿资产或其他未申报收入，需重点核查其现金持有情况。"
            )
        elif asset_reasonability["score"] >= 20:
            descriptions.append(
                f"该人员资产总额{asset_reasonability['total_assets'] / 10000:.2f}万元，是其年度工资收入的{asset_reasonability['assets_to_salary_ratio']:.1f}倍，资产积累较高，需关注其资产来源。"
            )
        elif asset_reasonability["score"] < 0:
            descriptions.append(
                f"该人员资产总额{asset_reasonability['total_assets'] / 10000:.2f}万元，低于其年度工资收入{asset_reasonability['assets_to_salary_ratio']:.1f}倍，可能存在大量隐匿资产（现金、未申报房产等），需重点核查其现金持有情况。"
            )
...
def quick_analyze_financial_risk(
    person_profile: Dict[str, Any],
    person_transactions: pd.DataFrame,
    income_yearly: Dict[int, float],
    property_data: Optional[List[Dict]] = None,
...
def analyze_all_persons_financial_risk(
    ...
    all_persons = set(profiles.keys()) - set(
        p
        for p in profiles.keys()
        if "公司" in p or "实业" in p or "机电" in p or "机械" in p
    )
    ...
    for person in all_persons:
        ...
        try:
            ...
            logger.info(
                f"✅ {person} 理财风险分析完成: 风险等级={result['risk_level']}, 评分={result['risk_score']}"
            )
        except Exception as e:
            logger.error(f"❌ {person} 理财风险分析失败: {str(e)}")
            results[person] = {"risk_level": "未知", "risk_score": 0, "error": str(e)}
