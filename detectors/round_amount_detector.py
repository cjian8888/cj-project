...
class RoundAmountDetector(BaseDetector):
    ...
    def detect(
        ...
        results = []
        results.extend(
            self._create_round_amount_suspicions(
                round_amount_txs, entity_name, min_occurrences, round_unit
            )
        )
        results.extend(
            self._create_lucky_number_suspicions(
    ...
    def _create_round_amount_suspicions(
        ...
        # 找出最常见的整数金额
        top_amounts = sorted(
            amount_groups.items(), key=lambda x: len(x[1]), reverse=True
        )[:3]
        top_amounts_str = ", ".join(
            [f"{amt:,.0f}元({len(txs)}笔)" for amt, txs in top_amounts]
        )
    ...
    def _create_lucky_number_suspicions(
        self, transactions: List[Dict], entity_name: str, min_occurrences: int
    ) -> List[Dict[str, Any]]:
        """创建吉利数字金额疑点。"""
        if len(transactions) < min_occurrences:
            return []
