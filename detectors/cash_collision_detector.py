...
class CashCollisionDetector(BaseDetector):
    ...
    def detect(self, data: Dict[str, Any], config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for entity_name, df in cleaned_data.items():
            ...
            single_collisions = self._detect_single_entity_collision(
                withdrawals, deposits, entity_name, 
                cash_time_window_hours, amount_tolerance_ratio
            )
    ...
    def _detect_single_entity_collision(
        ...
        # 应用阈值筛选
        time_mask = merged['hours_diff'] <= time_window_hours
        amount_mask = merged['amount_ratio'] <= amount_tolerance
        ...
        if not valid_collisions.empty:
            for _, row in valid_collisions.iterrows():
                ...
                collisions.append({
                    ...
                    'time_diff_hours': round(row['hours_diff'], 2),
                    'withdrawal_amount': row['amount_wd'],
                    'deposit_amount': row['amount_dp'],
                    'amount_diff': round(row['amount_diff_abs'], 2),
