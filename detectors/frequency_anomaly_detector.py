...
class FrequencyAnomalyDetector(BaseDetector):
    ...
    def detect(self, data: Dict[str, Any], config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # 过滤小额交易
        filtered_txs = [tx for tx in parsed_transactions if abs(tx["amount"]) >= min_amount]
        ...
        # 检测滑动窗口高频
        window_anomalies = self._detect_sliding_window(filtered_txs, window_hours, window_tx_threshold)
    ...
    def _detect_daily_frequency(self, transactions: List[Dict], threshold: int) -> List[Dict]:
        """检测单日高频交易。"""
    ...
    def _detect_hourly_frequency(self, transactions: List[Dict], threshold: int) -> List[Dict]:
        """检测单小时高频交易。"""
    ...
    def _detect_sliding_window(
        ...
        for i, tx in enumerate(transactions):
            ...
            if len(window_txs) >= threshold:
                total_amount = sum(abs(t["amount"]) for t in window_txs)
                anomalies.append({
        ...
        for anomaly in anomalies:
            ...
            for existing in unique_anomalies:
                if set(id(t) for t in anomaly["transactions"]) <= set(id(t) for t in existing["transactions"]):
                    is_subset = True
                    break
            if not is_subset:
    ...
    def _create_daily_suspicions(self, anomalies: List[Dict], entity_name: str) -> List[Dict[str, Any]]:
        """创建单日高频交易疑点。"""
        if not anomalies:
            return []
        ...
        for i, anomaly in enumerate(anomalies[:3]):
            ...
            severity = SuspicionSeverity.HIGH.value if tx_count >= 20 else SuspicionSeverity.MEDIUM.value
    ...
    def _create_hourly_suspicions(self, anomalies: List[Dict], entity_name: str) -> List[Dict[str, Any]]:
        """创建单小时高频交易疑点。"""
        if not anomalies:
            return []
        ...
        for i, anomaly in enumerate(anomalies[:3]):
            ...
            severity = SuspicionSeverity.HIGH.value if tx_count >= 10 else SuspicionSeverity.MEDIUM.value
    ...
    def _create_window_suspicions(self, anomalies: List[Dict], entity_name: str) -> List[Dict[str, Any]]:
        """创建滑动窗口高频交易疑点。"""
        if not anomalies:
            return []
