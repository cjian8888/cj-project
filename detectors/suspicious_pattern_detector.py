...
class SuspiciousPatternDetector(BaseDetector):
    ...
    def detect(
        ...
        results.extend(
            self._create_scatter_gather_suspicions(scatter_gather, entity_name)
        )
        ...
        results.extend(
            self._create_gather_scatter_suspions(gather_scatter, entity_name)
        )
    ...
    def _detect_scatter_gather(
        ...
        for tx_date, sources in daily_inflows.items():
            if len(sources) >= min_sources:
                total_inflow = sum(
                    sum(t.get("amount") or 0 for t in txs) for txs in sources.values()
                )
                ...
                if outflows:
                    total_outflow = sum(abs(tx.get("amount") or 0) for tx in outflows)
                    anomalies.append(
                        {
                            "date": tx_date,
                            ...
                            "outflow_targets": list(
                                set(tx["counterparty"] for tx in outflows)
                            ),
                            "inflow_transactions": [
                                tx for txs in sources.values() for tx in txs
                            ],
    ...
    def _detect_gather_scatter(
        ...
        for tx_date, targets in daily_outflows.items():
            if len(targets) >= min_targets:
                total_outflow = sum(
                    sum(abs(t.get("amount") or 0) for t in txs)
                    ...
                    anomalies.append(
                        {
                            ...
                            "inflow_sources": list(
                                set(tx["counterparty"] for tx in inflows)
                            ),
                            "inflow_transactions": inflows,
                            "outflow_transactions": [
                                tx for txs in targets.values() for tx in txs
                            ],
                        }
                    )
                    ...
                    anomalies.append(
                        {
                            ...
                            "time_diff_hours": (
                                outflows[-1]["datetime"] - inflow_time
                            ).total_seconds()
                            / 3600,
    ...
    def _create_scatter_gather_suspicions(
    ...
    def _create_gather_scatter_suspions(
        self, anomalies: List[Dict], entity_name: str
    ) -> List[Dict[str, Any]]:
        """创建集中转入分散转出疑点。"""
        if not anomalies:
    ...
    def _create_fast_in_out_suspicions(
        self, anomalies: List[Dict], entity_name: str
    ) -> List[Dict[str, Any]]:
        """创建快进快出疑点。"""
        if not anomalies:
