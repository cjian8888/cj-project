...
def identify_salary_transactions(df: pd.DataFrame, entity_name: str) -> Tuple[pd.DataFrame, Dict]:
    """
    识别工资性收入交易（重构版）
    ...
    negative_mask = df['description'].apply(
        lambda x: utils.contains_keywords(str(x), config.EXCLUDED_REIMBURSEMENT_KEYWORDS)
    )
    salary_by_payer = df[(df['income'] > 0) & known_payer_mask & (~negative_mask)].copy()
    ...
    # 排除理财相关
    df_income = df_income[~df_income['description'].apply(
        ...
        for cp, cp_df in df_income.groupby('counterparty'):
            ...
            if len(dates) >= 2:
                ...
                if 25 <= avg_interval <= 35:
                    ...
                    if mean_amt > 0:
                        cv = (sum((x - mean_amt)**2 for x in amounts) / len(amounts))**0.5 / mean_amt
    ...
    # 合并所有工资收入
    all_salary_transactions = pd.concat([salary_by_strong, salary_by_payer, salary_by_regular])
    
    # 重新排序
    all_salary_transactions = all_salary_transactions.sort_values('date').reset_index(drop=True)
    ...
    logger.info(f'✓ 识别到工资交易 {len(all_salary_transactions)} 笔 (总计: {utils.format_currency(stats["total_amount"])})')
...
def classify_income(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    classified['interest'] = pd.concat([classified['interest'], df[small_interest_mask]])
    
    # 3. 已分类的交易从 other 中移除
    all_classified = pd.concat([classified['salary'], classified['wealth'], classified['interest']])
...
def analyze_income_structure(df: pd.DataFrame, entity_name: str) -> Dict:
    ...
    wealth_total = classified['wealth']['income'].sum() if not classified['wealth'].empty else 0
    interest_total = classified['interest']['income'].sum() if not classified['interest'].empty else 0
    other_total = classified['other']['income'].sum() if not classified['other'].empty else 0
    ...
    logger.info(f'✓ 收入分析完成: 总收入 {utils.format_currency(total_income)}, 工资收入占比 {salary_ratio:.1%}')
