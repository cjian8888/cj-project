...
_SERVER_PID_FILE_ENV = "FPAS_SERVER_PID_FILE"
...
def _server_pid_file_path() -> Optional[Path]:
    raw_path = os.environ.get(_SERVER_PID_FILE_ENV, "").strip()
    if not raw_path:
        return None
    try:
...
def _write_server_pid_file() -> None:
    pid_path = _server_pid_file_path()
    if pid_path is None:
        return
    try:
...
def _cleanup_server_pid_file() -> None:
    pid_path = _server_pid_file_path()
    if pid_path is None or not pid_path.exists():
        return
    try:
        current_pid = pid_path.read_text(encoding="ascii").strip()
    except OSError:
        return
...
_write_server_pid_file()
atexit.register(_cleanup_server_pid_file)
...
def _get_windows_cmd_executable() -> str:
    """返回可用的 cmd.exe 绝对路径。"""
    candidates = [
        os.environ.get("COMSPEC", ""),
        os.path.join(os.environ.get("SystemRoot", ""), "System32", "cmd.exe"),
        r"C:\Windows\System32\cmd.exe",
        "cmd.exe",
    ]
    for candidate in candidates:
        if candidate and (candidate.lower().endswith("cmd.exe") or candidate.lower() == "cmd.exe"):
            if candidate.lower() == "cmd.exe" or os.path.exists(candidate):
                return candidate
    return "cmd.exe"


def _format_windows_shell_execute_error(result_code: int) -> str:
    """将 ShellExecuteW 错误码转换为可读文本。"""
    known_errors = {
        0: "系统资源不足，无法完成目录打开操作",
        2: "找不到指定文件",
        3: "找不到指定路径",
    ...
    return known_errors.get(
        int(result_code),
        f"ShellExecuteW 返回错误码 {int(result_code)}",
    )
...
def _shell_execute_windows(
    ...
    *,
    ...
    """调用 Windows ShellExecuteW，优先走资源管理器原生目录打开能力。"""
    import ctypes
    ...
    if result_code <= 32:
        raise OSError(_format_windows_shell_execute_error(result_code))


def _open_folder_in_windows(folder_path: str) -> None:
    """在 Windows 上尽量稳定地打开目录。"""
    target_path = os.path.realpath(os.path.abspath(str(folder_path)))
    ...
    def _record_failure(strategy: str, exc: Exception) -> None:
        error_text = f"{strategy}: {exc}"
        errors.append(error_text)
        logger.warning(f"Windows 打开目录失败 [{strategy}]: path={target_path}, error={exc}")

    for verb in ("explore", "open"):
        try:
            _shell_execute_windows(
                target_path,
                verb=verb,
                working_dir=os.path.dirname(target_path) or None,
            )
            return
        except Exception as exc:
            _record_failure(f"ShellExecute({verb})", exc)

    explorer_executable = _get_windows_explorer_executable()
    if explorer_executable:
        try:
            _shell_execute_windows(
                explorer_executable,
                parameters=f'"{target_path}"',
                working_dir=os.path.dirname(explorer_executable) or None,
            )
            return
        except Exception as exc:
            _record_failure("ShellExecute(explorer.exe)", exc)

        try:
            subprocess.Popen([explorer_executable, target_path])
            return
        except Exception as exc:
            _record_failure("subprocess(explorer.exe)", exc)

    try:
        subprocess.Popen([_get_windows_cmd_executable(), "/c", "start", "", target_path])
        return
    except Exception as exc:
        _record_failure("subprocess(cmd start)", exc)

    if errors:
        raise OSError("Windows 目录打开失败: " + "；".join(errors))
    raise OSError("未找到可用的 Windows 文件管理器")


def _select_directory_windows_native(title: str, initial_dir: str) -> str:
    """使用 Win32 原生目录选择器选择目录，优先保证 Win7/便携运行时可用。"""
    import ctypes
    from ctypes import wintypes
    ...
    display_name = ctypes.create_unicode_buffer(MAX_BUFFER)
    initial_dir_buffer = ctypes.create_unicode_buffer(initial_dir)
    initial_dir_ptr = ctypes.cast(initial_dir_buffer, ctypes.c_void_p).value or 0

    @BFFCALLBACK
    def _browse_callback(hwnd, msg, _lparam, _lpdata):
        if msg == BFFM_INITIALIZED and initial_dir_ptr:
            user32.SendMessageW(hwnd, BFFM_SETSELECTIONW, 1, initial_dir_ptr)
        return 0
    ...
    browse_info.lpfn = _browse_callback
    ...
    selected_pidl = None
    try:
        selected_pidl = shell32.SHBrowseForFolderW(ctypes.byref(browse_info))
        if not selected_pidl:
            return ""
        path_buffer = ctypes.create_unicode_buffer(MAX_BUFFER)
        if not shell32.SHGetPathFromIDListW(selected_pidl, path_buffer):
            return ""
        return str(path_buffer.value or "").strip()
    finally:
        if selected_pidl:
            ole32.CoTaskMemFree(selected_pidl)
        if coinit_result in (0, 1):
            ole32.CoUninitialize()


def _select_directory_windows_tk(title: str, initial_dir: str) -> str:
    """使用 Tk 作为 Windows 目录选择兜底。"""
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "deliveryMode": _is_delivery_mode(),
            },
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
...
async def select_directory(request: DirectorySelectRequest):
    ...
    try:
        if system == "Darwin":  # macOS
            ...
            # Win7/Win11 优先走原生 Win32 目录选择器；Tk 仅作为兜底。
            try:
                fallback_dir = (
                    _get_active_input_dir()
                ...
                try:
                    selected_path = _select_directory_windows_native(title, initial_dir)
                except Exception as shell_exc:
                    logging.getLogger(__name__).warning(
                        f"Windows 原生目录选择失败，回退 tkinter: {shell_exc}"
                    )
                    selected_path = _select_directory_windows_tk(title, initial_dir)

                if selected_path:
                    return {"success": True, "path": selected_path}
                return {"success": False, "error": "用户取消了选择"}
            except Exception as e:
                logging.getLogger(__name__).warning(f"Windows 目录选择失败: {e}")
                return {"success": False, "error": f"Windows 目录选择失败: {e}"}
        else:
...
async def open_folder(request: OpenFolderRequest):
    ...
    try:
        ...
        if system == "Darwin":  # macOS
            ...
            _open_folder_in_windows(folder_path)
        else:  # Linux
