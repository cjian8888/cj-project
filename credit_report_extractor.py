...
def parse_credit_file(file_path: str) -> Dict[str, Dict]:
    ...
    # 判断是个人还是企业
    entity_type = "company" if len(id_number) == 18 and not id_number[0].isdigit() else "person"
        ...
        for i, row in df.iterrows():
            if pd.notna(row.iloc[0]) and "账户数合计" in str(row.iloc[0]):
                ...
                if i + 1 < len(df):
                    ...
                    summary["total_accounts"] = _safe_int(data_row.iloc[0])
                    summary["business_types"] = _safe_int(data_row.iloc[1])
...
def _parse_loan_accounts(xls: pd.ExcelFile) -> tuple:
    ...
    try:
        ...
        for i in range(header_row + 1, len(df)):
            ...
            # 检查是否是新的段落标题
            if pd.notna(row.iloc[0]) and any(x in str(row.iloc[0]) for x in ["信息段", "信息单元"]):
                break
            
            account = {
                "account_id": _safe_str(row.iloc[0]),
                "account_type": _safe_str(row.iloc[1]),
                "institution_type": _safe_str(row.iloc[2]),
                "institution_code": _safe_str(row.iloc[3]),
                "business_type": _safe_str(row.iloc[6]) if len(row) > 6 else None,
                "open_date": _safe_date(row.iloc[7]) if len(row) > 7 else None,
                "currency": _safe_str(row.iloc[8]) if len(row) > 8 else None,
                "amount": _safe_float(row.iloc[9]) if len(row) > 9 else None,
                "credit_limit": _safe_float(row.iloc[10]) if len(row) > 10 else None,
            }
...
def get_credit_alerts(data_dir: str) -> List[Dict]:
    ...
    for id_number, info in credit_data.items():
        ...
        # 检查不良公共记录
        for key in ["tax_arrears_count", "civil_judgments_count", 
                    "enforcements_count", "penalties_count"]:
...
if __name__ == "__main__":
    ...
    print(f"\n=== 征信预警 ({len(alerts)} 条) ===")
    for alert in alerts:
        print(f"  {alert.get('name')}: {alert.get('alert_type')} ({alert.get('count')}次)")
